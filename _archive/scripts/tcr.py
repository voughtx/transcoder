...
def parse_tokens() -> List[str]:
    # Preferred: BOT_TOKENS_JSON='["token1","token2",...]'
    ...
    # Alternative: BOT_TOKENS='token1,token2,...'
    ...
    # Fallback: BOT_TOKEN_1..BOT_TOKEN_200
    tokens = []
...
def build_ffmpeg_cmd(direct: str, out_dir: Path, seg_time: int, video_mode: str, audio_mode: str, container: str = "fmp4") -> List[str]:
    index = out_dir / "index.m3u8"
    if container == "mpegts":
        seg_tpl = str(out_dir / "seg_%05d.ts")
        seg_type_args = ["-hls_segment_type", "mpegts"]
    else:
        seg_tpl = str(out_dir / "seg_%05d.m4s")
        seg_type_args = ["-hls_segment_type", "fmp4", "-hls_fmp4_init_filename", "init.mp4"]
    base = [
    ...
    hls_args = ["-f", "hls", "-hls_time", str(seg_time), "-hls_playlist_type", "vod", *seg_type_args, "-hls_flags", "independent_segments", "-hls_segment_filename", seg_tpl, str(index)]
...
def attempt_live_transcode_upload(direct: str, title: str, video_url: str, seg_time: int, video_mode: str, audio_mode: str, chosen_worker: str, tokens: List[str], chat_id: str, hard_bytes: int, max_parallel: int, container: str = "fmp4"):
    out_dir = Path("hls")
    if out_dir.exists():
    ...
    cmd = build_ffmpeg_cmd(direct, out_dir, seg_time, video_mode, audio_mode, container)
    print("CMD:", " ".join(cmd))
    ...
    try:
        while True:
            ...
            # Upload init once available (sirf fMP4 me init banta hai; TS me init_uri None rahega)
        ...
        init_uri, entries, original = parse_hls_current(index)
        # container-aware: TS me init nahi hota (init_meta None = OK), fMP4 me zaroori hai
        if container == "fmp4" and not init_meta:
        ...
        playlist = build_worker_playlist(original, init_meta["worker_url"] if init_meta else None, [x["worker_url"] for x in segments])
        max_size = max([init_meta.get("s", 0) if init_meta else 0] + [x.get("s", 0) for x in segments])
...
def main():
    ...
    container = env("CONTAINER", "fmp4").lower()   # "fmp4" (default) | "mpegts" (TS)
    ...
    for st in seg_times:
        ...
        try:
            init_meta, segments, playlist, max_size, uploaded_all = attempt_live_transcode_upload(direct, title, video_url, st, video_mode, audio_mode, chosen_worker, tokens, chat_id, hard_bytes, max_parallel, container)
